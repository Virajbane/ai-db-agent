  ...
  async function send() {
    ...
    try {
      const res = await fetch("/api/ai", {
        method: "POST",
        body: JSON.stringify({ dbType: "mongodb", userText, collections: [], previewLimit: 50 }),
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-md">
                  {["Find all users", "Count records", "Find active items", "List recent posts"].map((q) => (
                    <button
                      key={q}
                      onClick={() => setInput(q)}
                      className="px-4 py-3 rounded-lg bg-neutral-900 hover:bg-neutral-800 border border-neutral-800 text-gray-300 hover:text-white transition text-sm font-medium"
                    >
                      {q}
                    </button>
                  ))}
                      ...
                      {m.action && (
                        <div className="mt-3 flex gap-2 flex-wrap">
                          <button
                            ...
                            disabled={executing}
                            className="px-2.5 py-1 text-xs rounded bg-neutral-800 hover:bg-neutral-700 text-gray-300 hover:text-white border border-neutral-700 disabled:opacity-50 transition"
                          >
                            Preview
                          </button>
                          <button
                          ...
                          <button
                            ...
                            className="px-2.5 py-1 text-xs rounded bg-neutral-800 hover:bg-neutral-700 text-gray-300 hover:text-white border border-neutral-700 transition"
                          >
                            Copy
                          </button>
                        </div>
          ...
          <div className="max-w-4xl mx-auto w-full px-4 sm:px-6 py-4 sm:py-6">
            <div className="flex gap-3">
              <input
                ...
                className="flex-1 px-4 py-3 rounded-lg bg-neutral-900 border border-neutral-800 hover:border-neutral-700 text-white placeholder-gray-600 focus:outline-none focus:border-white focus:ring-1 focus:ring-white/20 transition text-sm disabled:opacity-50"
            ...
            <p className="text-xs text-gray-600 mt-2">Press Enter to send</p>
          </div>
        </div>
      </div>
    </div>
